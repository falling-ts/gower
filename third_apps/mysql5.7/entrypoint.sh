...
EOF
    ...
    if [ -z "$MYSQL_DATABASE_CHAR" ]; then
      MYSQL_DATABASE_ENCODING="utf8mb4"
      MYSQL_DATABASE_ENCODING_SORT="utf8mb4_unicode_ci"
      echo "没有设置数据库编码."
    else
      IFS=',' read -ra parts <<< "$MYSQL_DATABASE_CHAR"
      MYSQL_DATABASE_ENCODING="${parts[0]}"
      MYSQL_DATABASE_ENCODING_SORT="${parts[1]}"
      echo "设置了数据库编码:${parts[0]}, 排序规则:${parts[1]}."
    fi

		if [ "$MYSQL_DATABASE" ]; then
			echo "CREATE DATABASE IF NOT EXISTS \`$MYSQL_DATABASE\` CHARACTER SET $MYSQL_DATABASE_ENCODING COLLATE $MYSQL_DATABASE_ENCODING_SORT;" | "${mysql[@]}"
